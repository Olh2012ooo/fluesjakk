...
const PIECE_NAME = { p: 'bonde', n: 'springer', b: 'løper', r: 'tårn', q: 'dronning', k: 'konge' };
// Bestemt form og eiendomsord, så setningene blir naturlige («tårnet ditt», ikke «din tårn»).
const PIECE_BESTEMT = { p: 'bonden', n: 'springeren', b: 'løperen', r: 'tårnet', q: 'dronningen', k: 'kongen' };
const PIECE_EIENDOM = { p: 'din', n: 'din', b: 'din', r: 'ditt', q: 'din', k: 'din' };
const PIECE_EGEN = { p: 'sin egen', n: 'sin egen', b: 'sin egen', r: 'sitt eget', q: 'sin egen', k: 'sin egen' };
// Etter «egen» står brikken i ubestemt form: «sin egen konge», ikke «sin egen kongen».
...
export function seesColor(sq, board, flyColor) {
  ...
  if (!piece) {
    ...
    return light ? [96, 110, 122, 0.5] : [40, 48, 56, 0.8];
  }
  const level = PIECE_LEVEL[piece.type] || 0.7;
  return piece.color === flyColor ? [255, 180, 84, 0.45 + 0.55 * level] : [108, 198, 234, 0.45 + 0.55 * level];
}
...
/** «Fluas høyre øye har festet seg på springeren din på f6» — fra et blikkbytte, ruten det så på og fargen dens. */
export function gazeLine(shift, board, flyColor, rnd = Math.random) {
  ...
  const what = !piece
    ? `den tomme ruten ${name}`
    : piece.color === flyColor
      ? `${PIECE_EGEN[t]} ${PIECE_NAME[t]} på ${name}`
      : `${PIECE_BESTEMT[t]} ${PIECE_EIENDOM[t]} på ${name}`;
  const side = shift.eye ? 'høyre' : 'venstre';
  const lines = [
    `Fluas ${side} øye har festet seg på ${what}.`,
    `Det ${side} øyet lyste opp over ${what}.`,
    `Fotoreseptorene i det ${side} øyet dirrer mot ${what}.`,
  ];
...
export class EyePanel {
  constructor(root) {
    ...
    this.tip = root.querySelector('.oye-tip');
    this.caption = root.querySelector('.oye-tekst');
    ...
    this.onModeChange = () => {};
    root.querySelectorAll('[data-oye]').forEach((b) => b.addEventListener('click', () => this.setMode(b.dataset.oye)));
  ...
  setRetina(retina) {
    ...
    if (this.hasRetina) {
      ...
      this.files = `venstre øye ser linjene ${files.left} · høyre øye ser ${files.right}`;
      this._relayout();
    }
    this._caption();
    this._draw();
  ...
  setMode(mode) {
    if (mode !== 'ser' && mode !== 'foler') return;
    this.mode = mode;
    this.root.querySelectorAll('[data-oye]').forEach((b) => b.setAttribute('aria-pressed', String(b.dataset.oye === mode)));
  ...
  _caption() {
    ...
    if (!this.hasRetina) {
      this.caption.textContent = 'Denne modellen har ingen retina — flua kjenner brettet gjennom 2 048 sanseceller i stedet.';
      return;
    }
    if (this.mode === 'ser') {
      this.caption.textContent = `${this.retina.n.toLocaleString('nb-NO')} fotoreseptorer ser brettet fra fluas egen side. Gult er fluas brikker, blått er dine. ${this.files}`;
    } else {
      this.caption.textContent = this.drive
        ? `Inngangsstrømmen til hver fotoreseptor (w_ret · brett + b_ret) ${this.driveLabel}. Grønt er stimulerende, rødt er hemmende, skalert til dette bildet.`
        : 'Inngangsstrømmen til hver fotoreseptor — flua har ikke sett på brettet ennå.';
    }
  }
  ...
  _hover(e) {
    ...
    const what = piece ? ` · ${PIECE_BESTEMT[piece.type]} ${piece.color === this.flyColor ? 'til flua' : 'din'} ${PIECE_GLYPH[piece.type]}` : ' · tom rute';
    const drive = this.drive ? ` · strøm ${this.drive[best] >= 0 ? '+' : ''}${this.drive[best].toFixed(2)}` : '';
    this.tip.textContent = `${R.legend[R.type[best]] || 'R?'} · ${R.eye[best] ? 'høyre' : 'venstre'} øye · ser ${squareName(real)}${what}${drive}`;
  ...
  _draw() {
    ...
    for (const b of L.boxes) {
      ...
      ctx.strokeStyle = 'rgba(131, 140, 148, .22)'; ctx.lineWidth = 1; ctx.stroke();
    }
    ...
    const r = L.r;
    if (pulsing) {   // et svakt gult skjær bak fotoreseptorene som ser siste trekks ruter
      ctx.fillStyle = `rgba(255, 180, 84, ${0.22 * pulseK * (0.7 + 0.3 * Math.sin(now / 90))})`;
      for (let k = 0; k < R.n; k++) {
    ...
    for (let k = 0; k < R.n; k++) {
      ...
      if (hoverSq >= 0 && R.square[k] === hoverSq) {
        ...
        ctx.strokeStyle = k === this.hover ? '#b6f04a' : 'rgba(182, 240, 74, .5)'; ctx.lineWidth = k === this.hover ? 1.5 : 1; ctx.stroke();
      }
    }
    // øyemerker
    ctx.font = '600 10px ui-monospace, Menlo, Consolas, monospace';
    ctx.fillStyle = 'rgba(131, 140, 148, .85)'; ctx.textAlign = 'left';
    if (L.boxes[0]) ctx.fillText('VENSTRE ØYE', 6, this.h - 5);
    ctx.textAlign = 'right';
    if (L.boxes[1]) ctx.fillText('HØYRE ØYE', this.w - 6, this.h - 5);
  }
}
